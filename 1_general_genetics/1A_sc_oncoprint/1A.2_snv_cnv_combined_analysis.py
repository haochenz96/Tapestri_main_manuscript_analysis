...
pt_h5 = list((data_dir / "fillout_h5").glob( f"{patient_name}*.h5"))[0]
...
# +++++ fix for single-sample cases +++++
if not all([i in pt.dna.barcodes() for i in pt.cnv.barcodes()]):
    print("DNA and CNV barcodes do not match. Fixing it.")
    pt.cnv.add_row_attr("barcode", pt.cnv.barcodes() + '-' + pt.cnv.row_attrs["sample_name"])

# %% Add raw FALCON results to H5
# HZ refined
cn_assignment_f = list((condor_pipeline_dir / "condor_downstream/ete_trees_refined_subclonal_snvs" / patient_name).glob(f"{patient_name}*assignment.csv"))[0]
...
# add CN_clone-0
cn_clone_palette["0"] = np.array(px.colors.qualitative.Set3)[0]
# rename the keys
...
cn_assignment_df["sample"] = cn_assignment_df.index
cn_assignment_df.set_index('cell_barcode_formatted', inplace=True)
# # +++++ for single-sample case only: +++++
# cn_assignment_df.set_index('cell_barcode', inplace=True)
# cn_assignment_df = cn_assignment_df[cn_assignment_df["sample"] == patient_name]

# # ++++++++++++++++++++++++++++++++++++++++
pt.dna.row_attrs['label'] = np.array(list(
    map(
        lambda x: f"CN_clone-{int(cn_assignment_df.loc[x, 'final_clone_id'])}" if x in cn_assignment_df.index else "CN_clone-0", 
        pt.dna.barcodes())
    ))
...
attribute="AF_MISSING"
# also make sure to retrieve th|e sorted_bars 
del pt.dna.__dict__["_Assay__heatmap"]
...
sc_cnv_heatmap = pt.cnv.heatmap(
    ...
    features = ['CDKN2A', 'RREB1', 'KRAS', 'BRCA2', 'SMAD6', 'TP53', 'SMAD4'],
