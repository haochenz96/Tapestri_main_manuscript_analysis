...
sample_name_map = pd.read_excel(
    '/data/iacobuzc/haochen/Tapestri_batch3/Tapestri_batch2_samples_MASTER.xlsx',
    sheet_name='all_sample_clinical_info',
    ).set_index('sample')['HZ_official_site_name'].to_dict()

# %% Read in data
pickle_dir = Path("../condor_outputs/pickle_files")
manual_snv_dir = Path(
    "/data/iacobuzc/haochen/Tapestri_batch3/batch3_data_compiled/manual_annotated_snv_lists"
)
output_dir = Path("/data/iacobuzc/haochen/Tapestri_batch3/analysis/condor-pipeline/condor_downsteam/ete_trees_refined_subclonal_snvs")
...
leaves_to_merge_to_diploid = merge_clones_into_diploid(
    ...
    rm_clones_with_no_somatic_snv_events=True, # @HZ 2024-04-26
)
...
for cn_clone_name, cn_clone_node in subclonal_trees.items():
    if cn_clone_name in leaves_to_merge_to_diploid:
        logger.warning(f"clone {cn_clone_name} has been merged to diploid clone. Skipping...")
        continue
...
"""
for node in ete_tree_with_subclonal_snvs.traverse("preorder"):
    if node.is_root():
        continue
    else:
        node.dist = (
