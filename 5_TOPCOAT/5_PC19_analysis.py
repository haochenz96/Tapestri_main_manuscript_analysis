...
if "_Assay__heatmap" in pt.dna.__dict__:
...
for sample_i in sample_names:
    ...
    # save the figure
    fig.write_image(
        str(output_dir / f"{sample_i}_sc_heatmap_DNA.png"), 
        width = 500 + 10 * len(snv_df.index.tolist()), scale=3
    )
    print(f'[INFO] Saved heatmap for {sample_i} to {output_dir / f"{sample_i}_DNA_heatmap_tumor_cells.png"}.')
# %% ===== plot distribution of TP53 and KRAS loci =====
tp53 = "chr17:7577559:G/A"
kras = "chr12:25398284:C/T"
# # distribution of myc in those having positive read in T1
# af_t0 = ss_tumors["TP49-IR-0T"].dna.get_attribute("AF_MISSING", features = [tp53, kras], constraint="row")
# af_t0 = af_t0.sort_values(by=[tp53, kras], ascending=False)
# af_t1 = ss_tumors["TP49-IR-1T"].dna.get_attribute("AF_MISSING", features = [tp53, kras], constraint="row")

for sample_i in ss_tumors:
    fig = plot_snv_clone(
            ss_tumors[sample_i],
            sample_name=sample_i,
            story_topic = f'{patient_name}-drivers',
            voi = [tp53, kras],
            attribute = "mut_filtered",
            barcode_sort_method = "hier",
            ann_map = snv_ann_map
        )
    fig.write_image(output_dir / f"{sample_i}_TP53_KRAS_mut_filtered.png", width = 500, scale=3)
