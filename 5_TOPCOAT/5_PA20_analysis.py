...
myc_af_t1 = ss_tumors["BPA-1-IR"].dna.get_attribute("AF_MISSING", features = [myc], constraint="row")
...
fig.savefig(output_dir / "BPA-1-IR_MYC_AF_distribution.pdf")

# %% ----- distribution of CHEK2 locus AF in all tumor cells
chek2_af_t0 = ss_tumors["BPA-1-RSX"].dna.get_attribute("AF_MISSING", features = [chek2], constraint="row")
chek2_af_t1 = ss_tumors["BPA-1-IR"].dna.get_attribute("AF_MISSING", features = [chek2], constraint="row")
...
fig.savefig(output_dir / "BPA-1_two_timepts_CHEK2_AF_distribution.pdf")
plt.close()
# %% ===== what's myc's sc-prev in T0? =====
ss_tumors["BPA-1-RSX"].dna.genotype_variants()
myc_ALT = ss_tumors["BPA-1-RSX"].dna.get_attribute("alt_read_count", features=[myc], constraint="row")

ss_tumors["BPA-1-IR"].dna.genotype_variants()
kras_myc_mut_genotype = ss_tumors["BPA-1-IR"].dna.get_attribute("mut_filtered", features=[kras, myc], constraint="row")
